...
public class AveragedPerceptronNewsBinaryClassifier : INewsBinaryClassifier
{
    private ITransformer Model { get; set; }
    public ClassifierMetrics Metrics { get ; private set; }

    public AveragedPerceptronNewsBinaryClassifier()
    {
        try
        {
    ...
    public void Train()
    {
        ...
        var estimator = Utils.Context.Transforms.Text.FeaturizeText(outputColumnName: "Features", inputColumnName: nameof(ModelInput.Text))
            .Append(Utils.Context.BinaryClassification.Trainers.AveragedPerceptron(labelColumnName: "Label", featureColumnName: "Features"))
            .Append(Utils.Context.BinaryClassification.Calibrators.Platt());
