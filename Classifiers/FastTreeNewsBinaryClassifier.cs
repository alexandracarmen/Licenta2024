...
public class FastTreeNewsBinaryClassifier : INewsBinaryClassifier
{
    private ITransformer Model { get; set; }
    public ClassifierMetrics Metrics { get; private set; }

    public FastTreeNewsBinaryClassifier()
    {
        try
        {
    ...
    public void Train()
    {
        ...
        var estimator = Utils.Context.Transforms.Text.FeaturizeText(outputColumnName: "Features", inputColumnName: nameof(ModelInput.Text))
            .Append(Utils.Context.BinaryClassification.Trainers.FastTree(labelColumnName: "Label", featureColumnName: "Features"));
