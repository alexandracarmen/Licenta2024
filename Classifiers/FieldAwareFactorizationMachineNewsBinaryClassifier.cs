...
public class FieldAwareFactorizationMachineNewsBinaryClassifier : INewsBinaryClassifier
{
    private ITransformer Model { get; set; }
    public ClassifierMetrics Metrics { get ; private set; }

    public FieldAwareFactorizationMachineNewsBinaryClassifier()
    {
        try
        {
    ...
    public void Train()
    {
        ...
        var estimator = Utils.Context.Transforms.Text.FeaturizeText(outputColumnName: "Features", inputColumnName: nameof(ModelInput.Text))
            .Append(Utils.Context.BinaryClassification.Trainers.FieldAwareFactorizationMachine(labelColumnName: "Label", featureColumnName: "Features"));
